...
export default function ContactForm({ cookieConsent }) {
  ...
  const handleRecaptchaChange = (token) => {
    // This function is called when reCAPTCHA verification succeeds
    setRecaptchaToken(token);
    setResponseMsg(""); // Clear any previous warning
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate reCAPTCHA
    if (cookieConsent && !recaptchaToken) {
      setResponseMsg("⚠️ Please complete the reCAPTCHA.");
      return;
    }

    const bodyData = { ...formData };
    if (recaptchaToken) bodyData.captcha_token = recaptchaToken;
    ...
    try {
      const res = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          access_key: "6a9e7033-bef8-4435-abc7-007bf34203af",
          ...bodyData,
        }),
      });
    ...
    <Layout>
      <form onSubmit={handleSubmit} noValidate>
        ...
        {cookieConsent && ReCAPTCHA ? (
          <ReCAPTCHA
            sitekey="YOUR_SITE_KEY_HERE" // Replace with your v2 key
      ...
      <style jsx>{`
        ...
        :global(.g-recaptcha) {
          display: none; // hide invisible reCAPTCHA
        }
      `}</style>
    </Layout>
  );
